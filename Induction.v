...
Proof.
  intros n m p.
  assert (H1: m+p =p+m). {rewrite -> plus_comm. reflexivity. }
  assert (H2: n+(p+m)=(n+p)+m). {rewrite ->plus_assoc''. reflexivity. }
  assert (H3: (n+p)+m =m+(n+p)). {rewrite -> plus_comm. reflexivity. }
  rewrite ->H1.
  rewrite ->H2.
  rewrite ->H3.
  reflexivity.
...
Theorem mult_distri :forall m n: nat,
  m + m * n = m * S n.
Proof.
  intros m n.
...
Proof.
  ...
  -simpl. rewrite <- mult_distri. rewrite -> IHn'. reflexivity. Qed.
...
(** [] *)Theorem nat_bin_nat_pres_incr: forall n: bin,
...
Proof.
  ...
  -simpl. reflexivity.
  -simpl. reflexivity.
  -simpl. rewrite ->IHn. rewrite <- plus_n_O. rewrite <- plus_n_O. rewrite ->plus_n_Sm. reflexivity.
Qed.
...
Proof.
  ...
  induction n.
  -simpl. reflexivity. 
  -simpl. rewrite -> nat_bin_nat_pres_incr. rewrite->IHn. reflexivity.
Qed.
