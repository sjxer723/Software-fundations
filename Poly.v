...
(** Using argument synthesis and implicit arguments, we can
    ...
    made the constructor type arguments implicit
, Coq will know to
...
Fixpoint split {X Y : Type} (l : list (X*Y))
  ...
  match l with
  |[] => pair [] []
  |(x,y)::l =>pair (x::(fst(split l))) (y::(snd(split l)))
end.
...
(** **** Exercise: 3 stars, standard (map_rev)  
    ...
    auxiliary lemma. *)
Lemma map_1: forall (X Y: Type) (f : X-> Y) ( m n : list X),
...
Proof.
  ...
  induction l.
  -reflexivity.
  -simpl. rewrite -> map_1. rewrite ->IHl. reflexivity.
Qed.
(** [] *)
...
(* FILL IN HERE *)
Definition second {X}{Y}(x:X) (y:Y): Y :=y.
Example fold_example4:
  fold second [true] 2 = 2.
Proof. reflexivity. Qed.
